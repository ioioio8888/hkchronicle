...
func handleMsgSetEvent(ctx sdk.Context, keeper Keeper, msg MsgSetEvent) sdk.Result {
	if !msg.Owner.Equals(keeper.GetOwner(ctx, msg.Event)) { // Checks if the the msg sender is the same as the current owner
		return sdk.ErrUnauthorized("Incorrect Owner").Result() // If not, throw an error
	}
...
func handleMsgBuyEvent(ctx sdk.Context, keeper Keeper, msg MsgBuyEvent) sdk.Result {
	if keeper.GetPrice(ctx, msg.Event).IsAllGT(msg.Bid) { // Checks if the the bid price is greater than the price paid by the current owner
		return sdk.ErrInsufficientCoins("Bid not high enough").Result() // If not, throw an error
	}
	if keeper.HasOwner(ctx, msg.Event) {
		err := keeper.CoinKeeper.SendCoins(ctx, msg.Buyer, keeper.GetOwner(ctx, msg.Event), msg.Bid)
		if err != nil {
			return sdk.ErrInsufficientCoins("Buyer does not have enough coins").Result()
		}
	} else {
	...
	keeper.SetOwner(ctx, msg.Event, msg.Buyer)
	keeper.SetPrice(ctx, msg.Event, msg.Bid)
	return sdk.Result{}
}

// Handle a message to delete evnet
func handleMsgDeleteEvent(ctx sdk.Context, keeper Keeper, msg MsgDeleteEvent) sdk.Result {
	if !keeper.IsNamePresent(ctx, msg.Event) {
		return types.ErrNameDoesNotExist(types.DefaultCodespace).Result()
	}
	if !msg.Owner.Equals(keeper.GetOwner(ctx, msg.Event)) {
		return sdk.ErrUnauthorized("Incorrect Owner").Result()
	}
